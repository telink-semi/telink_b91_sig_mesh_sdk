...
int 	ota_firmware_size_k = FW_SIZE_MAX_K;	// same with pm_8269.c
#endif
...
void mesh_ota_save_data(u32 adr, u32 len, u8 * data){
    ...
    #if DISTRIBUTOR_UPDATE_CLIENT_EN
	if (adr == 0){
	    fw_update_srv_proc.reboot_flag_backup = data[0x20];
...
u32 soft_crc32_ota_flash(u32 addr, u32 len, u32 crc_init,u32 *out_crc_type1_blk)
{
    ...
    while(len){
        ...
        #endif
        if(0 == addr){
            buf[0x20] = fw_update_srv_proc.reboot_flag_backup;
...
int is_valid_telink_fw_flag()
{
    ...
    u8 fw_flag[4] = {0};
    mesh_ota_read_data(0x20, sizeof(fw_flag), fw_flag);
